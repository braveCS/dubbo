    ...
    public void subscribe(URL url, NotifyListener listener) {
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(urls)) {
                ...
                logger.error("Failed to subscribe " + url + ", Using cached list: " + urls + " from cache file: " + getUrl().getParameter(FILE_KEY, System.getProperty("java.io.tmpdir") + "/dubbo-registry-" + url.getHost() + ".cache") + ", cause: " + t.getMessage(), t);/**##CHANGE BY CN.FFCS##**/
            } else {
                // If the startup detection is opened, the Exception is thrown directly.
                boolean check = getUrl().getParameter(Constants.CHECK_KEY, true)
                        && url.getParameter(Constants.CHECK_KEY, true);
